...
SELECT
  ...
  CASE
    ...
    WHEN using_lines_not_secured_personal_assets >= 0.60 AND using_lines_not_secured_personal_assets <= 1 THEN '03 Alto'
...
SELECT
  num_segmento,
  MIN(using_lines_not_secured_personal_assets) AS range_min,
  MAX(using_lines_not_secured_personal_assets) AS range_max
FROM
  segmentos
GROUP BY
  num_segmento
...
SELECT
  'using lines not secured personal assets' AS variable,
  num_segmento,
  total_count,
